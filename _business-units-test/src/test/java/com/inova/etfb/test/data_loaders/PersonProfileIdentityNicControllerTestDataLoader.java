...
import com.inova.dao.entities.PersonIdentityNic;
...
import com.inova.dao.repositories.PersonIdentityNicRepository;
import com.inova.dao.repositories.PersonRepository;

@Component
public class PersonProfileIdentityNicControllerTestDataLoader {
	@Autowired
	PersonRepository personRepository;
	@Autowired
	PersonIdentityNicRepository personIdentityNicRepository;
	@Autowired
	...
	public PersonIdentityNic insertPersonIdentityNic() {
		PersonIdentityNic empVerifRequest = getExistingPersonIdentityNic();

		if (empVerifRequest != null) {
			return empVerifRequest;
		} else {
			empVerifRequest = new PersonIdentityNic();
			Person person = insertPerson();
			empVerifRequest.setPersonId(person);
			empVerifRequest.setNicNo("888888888V");
			MstIdentityNicType nicType = new MstIdentityNicType();
			nicType.setId(1l);
			empVerifRequest.setNicTypeId(nicType);
			empVerifRequest.setCreatedAt(new Date());
			empVerifRequest.setUpdatedAt(new Date());
			return personIdentityNicRepository.save(empVerifRequest);
		}
	}
	...
	private PersonIdentityNic getExistingPersonIdentityNic() {
		List<PersonIdentityNic> list = personIdentityNicRepository.findAll(PageRequest.of(0, 1)).getContent();
		if(!list.isEmpty()) {
			return list.get(0);
		} else {
			return null;
